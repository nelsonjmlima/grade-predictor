...
export default function RepositoryRankingPage() {
  ...
  const getMetricValue = (repo: any, metricId: string) => {
    switch(metricId) {
      case "commit_count":
                  ...
                  <CardContent>
                    <div className="text-2xl font-bold">
                      {selectedMetric === "predicted_grade" ? "N/A" : 
                        formatMetricValue(
                          Math.round(
                            sortedRepositories.reduce((sum, repo) => {
                              const value = getMetricValue(repo, selectedMetric);
                              return typeof value === "number" ? sum + value : sum;
                            }, 0) / sortedRepositories.length
                          ),
                          selectedMetric
                        )
                      }
                    ...
                    <p className="text-xs text-muted-foreground">
                      Across {sortedRepositories.length} repositories
                    </p>
                  </CardContent>
                </Card>
